...
#ifdef __APPLE__
			// Cmd+Left/Right → Home/End (macOS line-navigation convention).
			if ((mods & GLFW_MOD_SUPER) && (action == GLFW_PRESS || action == GLFW_REPEAT)) {
				if (key == GLFW_KEY_LEFT) {
					sys->core->KeyEvent(KEY_HOME, KE_KEYDOWN);
					sys->core->KeyEvent(KEY_HOME, KE_KEYUP);
					return;
				}
				if (key == GLFW_KEY_RIGHT) {
					sys->core->KeyEvent(KEY_END, KE_KEYDOWN);
					sys->core->KeyEvent(KEY_END, KE_KEYUP);
					return;
				}
			}
#endif
...
#ifdef __APPLE__
	// ANGLE/EGL on macOS creates a CAMetalLayer with contentsScale=1.0 by default,
	// placing the logical-size drawable in the lower-left quarter of the Retina
	// physical framebuffer.  Fix all sublayer contentsScales, then re-query the
	// framebuffer size so vid.fbSize reflects the physical pixel dimensions.
	extern void PlatformFixRetinaLayer(GLFWwindow*);
	PlatformFixRetinaLayer(wnd);

	// Wire up trackpad pinch-to-zoom → KEY_MWHEELUP/DOWN events.
	extern void PlatformSetupMagnification(std::function<void(int, int)>);
	{
		auto* sysPtr = sys;
		PlatformSetupMagnification([sysPtr](int key, int type) {
			sysPtr->core->KeyEvent((keyList_e)key, (keyEvent_s)type);
		});
	}
#endif
...
#ifdef __APPLE__
	// After fixing contentsScale the OS will reflect the change on the next
	// frame, but glfwGetFramebufferSize may still return logical pixels for
	// EGL contexts.  If fbSize equals size but dpiScale > 1, correct fbSize
	// to the actual physical pixel dimensions.
	if (vid.fbSize[0] == vid.size[0] && vid.fbSize[1] == vid.size[1]
	        && sys->video->vid.dpiScale > 1.0f) {
		vid.fbSize[0] = (int)(vid.size[0] * sys->video->vid.dpiScale + 0.5f);
		vid.fbSize[1] = (int)(vid.size[1] * sys->video->vid.dpiScale + 0.5f);
	}
#endif
...
#ifdef __APPLE__
		// EGL/ANGLE on macOS may report logical pixels here too; correct if needed.
		if (width == vid.size[0] && height == vid.size[1] && vid.dpiScale > 1.0f) {
			width  = (int)(width  * vid.dpiScale + 0.5f);
			height = (int)(height * vid.dpiScale + 0.5f);
		}
#endif
		vid.fbSize[0] = width;
		vid.fbSize[1] = height;
...
void sys_video_c::GetRelativeCursor(int& x, int& y)
{
	...
	// GLFW reports logical (point) coordinates; convert to physical pixels so
	// that l_GetCursorPos's /dpiScale yields correct logical coords for Lua.
	x = (int)floor(xpos * vid.dpiScale);
	y = (int)floor(ypos * vid.dpiScale);
}

void sys_video_c::SetRelativeCursor(int x, int y)
{
	if (!initialised) return;
	// x/y are in physical pixels; GLFW expects logical (point) coordinates.
